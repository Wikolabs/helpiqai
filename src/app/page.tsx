...
export default function DemoPage() {
  ...
  const t = lang === "fr" ? {
    ...
    poweredBy: "Modèle :",
    note: "DEMO POC, aucune connexion réelle Zendesk, Intercom, Notion. L'IA invente la KB pour la démonstration.",
  } : {
    ...
    <div style={{ minHeight: "100vh", background: PAL.bg, color: PAL.txt1, display: "flex", flexDirection: "column" }}>
      <style>{`
        *, *::before, *::after { box-sizing: border-box; }
        ...
        .wk-textarea { width: 100%; padding: 12px 14px; border-radius: 6px; background: ${PAL.surface}; border: 1px solid ${PAL.border}; color: ${PAL.txt1}; font-family: inherit; font-size: 14px; line-height: 1.55; resize: vertical; min-height: 250px; transition: border-color .2s, background .2s; }
        .wk-textarea:focus { outline: none; border-color: ${PAL.accent}; background: ${PAL.surfaceHover}; }
        .wk-btn-primary { background: ${PAL.accent}; color: ${ON_ACCENT}; border: none; border-radius: 6px; padding: 13px 22px; font-weight: 700; font-size: 14px; cursor: pointer; font-family: inherit; transition: opacity .2s, transform .2s; display: inline-flex; align-items: center; gap: 8px; }
      ...
      <main style={{ flex: 1, padding: "32px", maxWidth: 1200, margin: "0 auto", width: "100%" }}>
        ...
        <div className="demo-grid" style={{ display: "grid", gridTemplateColumns: "380px 1fr", gap: 24 }}>
          <section style={{ background: PAL.surface, border: `1px solid ${PAL.border}`, borderRadius: 6, padding: 22 }}>
            <h2 style={{ fontSize: "0.92rem", color: PAL.txt2, fontWeight: 700, margin: "0 0 10px" }}>{t.inputLabel}</h2>
